...
echo "Extracting CFLAGS from $TOOLCHAIN_FILE..."
CFLAGS=$(grep -E 'set\(CMAKE_C_FLAGS' "$TOOLCHAIN_FILE" | sed -E 's/set\(CMAKE_C_FLAGS "(.*)"\)/\1/' | tr -d '\n')

TARGET_FLAGS=$(grep -E 'set\(TARGET_FLAGS' "$TOOLCHAIN_FILE" | sed -E 's/set\(TARGET_FLAGS "(.*)"\)/\1/' | tr -d '\n')

FINAL_CFLAGS="$CFLAGS $TARGET_FLAGS"

if [ -z "$FINAL_CFLAGS" ]; then
    echo "Error: Failed to extract CFLAGS from $TOOLCHAIN_FILE"
    exit 1
fi
...
read -p "Do you want to continue with these CFLAGS? (y/n) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "Aborting."
    exit 0
...
export RET_CFLAGS="$FINAL_CFLAGS"
echo "CFLAGS exported as RET_CFLAGS."
